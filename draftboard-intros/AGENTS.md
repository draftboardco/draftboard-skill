...
`SKILL.md` is the single source of truth for how to drive the Draftboard tools (warm intros, paths,
targets, connections). Setup is in [`references/setup.md`](./references/setup.md): connecting is one
address (`https://mcp.draftboard.com`) and a browser approval — on Codex,
`codex mcp add draftboard --url https://mcp.draftboard.com` followed by `codex mcp login draftboard`.
